<?php
    ...
    class Use_Planning {
        ...
        function findByHoraireSalle($id_horaire, $id_salle, $semaine){
            // Connexion à la base de données
            $bdd = new Connexion_bdd();

            // Requête SQL
            $requete = 'SELECT * FROM planning WHERE fk_id_horaire = ? AND fk_id_salle = ? AND numero_semaine = ?';

            // Exécution de la requête
            if(!$bdd->doQuery($requete, [$id_horaire, $id_salle, $semaine])){
                return false;
            } else {
                // Récupération des résultats
                if($plannings_recup = $bdd->tabResultat){
